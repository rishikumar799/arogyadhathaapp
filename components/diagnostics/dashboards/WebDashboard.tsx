...
export default function DiagnosticsDashboard() {
    ...
    <View 
      style={[
        ...
        { 
          ...
          transform: isHovered === `kpi-${item.label}` ? [{ scale: 1.03 }] : [{ scale: 1 }],
        }
      ]}
      onMouseEnter={() => handleMouseEnter(`kpi-${item.label}`)}
  ...
  const renderTestCategory = ({ item, index }) => (
    <View 
      style={[
        styles.categoryCard,
        {
          transform: isHovered === `category-${index}` ? [{ scale: 1.05 }] : [{ scale: 1 }],
          shadowOpacity: isHovered === `category-${index}` ? 0.15 : 0.05,
        }
      ]}
  ...
  const renderTest = ({ item }) => (
    <TouchableOpacity 
      style={[
        styles.testCard,
        {
          transform: isHovered === `test-${item.id}` ? [{ translateY: -2 }] : [{ translateY: 0 }],
          shadowOpacity: isHovered === `test-${item.id}` ? 0.15 : 0.05,
        }
      ]}
      onMouseEnter={() => handleMouseEnter(`test-${item.id}`)}
      onMouseLeave={handleMouseLeave}
    >
  ...
  const renderCriticalResult = ({ item, index }) => (
    <View 
      style={[
        styles.criticalCard,
        {
          transform: isHovered === `critical-${index}` ? [{ scale: 1.02 }] : [{ scale: 1 }],
        }
      ]}
      onMouseEnter={() => handleMouseEnter(`critical-${index}`)}
      onMouseLeave={handleMouseLeave}
    >
      ...
      <TouchableOpacity 
        ...
        onPress={() => console.log(`Notify doctor for ${item.patient}`)}
      >
  ...
  const renderEquipment = ({ item, index }) => (
    <View 
      style={[
        ...
        {
          transform: isHovered === `equipment-${index}` ? [{ translateY: -2 }] : [{ translateY: 0 }],
        }
      ]}
      onMouseEnter={() => handleMouseEnter(`equipment-${index}`)}
      onMouseLeave={handleMouseLeave}
    >
          ...
          style={[
            ...
            { 
              ...
              background: item.value > 60 
                ? `linear-gradient(to top, ${COLORS.primary}, ${COLORS.primaryLight})`
                : `linear-gradient(to top, ${COLORS.primaryLight}, ${COLORS.secondaryLight})`
            }
          ]} 
        />
      </View>
      ...
      <View 
        style={[
          ...
          { 
            ...
            transform: isHovered === `action-${index}` ? [{ scale: 1.1 }] : [{ scale: 1 }],
          }
        ]}
      >
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <StatusBar backgroundColor={COLORS.bannerMid} barStyle="light-content" />
      <ScrollView 
        style={styles.page} 
      ...
      >
        ...
        <View style={styles.section}>
          ...
          >
            {QUICK_ACTIONS.map((action, index) => renderActionCard({ item: action, index }))}
          </ScrollView>
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.categoriesGrid}>
            {TEST_CATEGORIES.map((category, index) => renderTestCategory({ item: category, index }))}
          </View>
        </View>
        ...
        <View style={styles.section}>
          <View style={styles.cardHeader}>
            ...
            <TouchableOpacity 
              ...
              onPress={() => console.log("View all tests")}
            >
          ...
          <View style={styles.testsGrid}>
            {TODAY_TESTS.map((test) => renderTest({ item: test }))}
          </View>
        </View>
        ...
        <View style={styles.section}>
          <View style={styles.cardHeader}>
            ...
            <View style={styles.criticalBadge}>
              <ThemedText style={styles.criticalCount}>{CRITICAL_RESULTS.length}</ThemedText>
            </View>
          </View>
          <FlatList 
            data={CRITICAL_RESULTS} 
            renderItem={renderCriticalResult} 
            keyExtractor={(item, index) => index.toString()} 
            horizontal
            showsHorizontalScrollIndicator={Platform.OS === 'web'}
            contentContainerStyle={styles.criticalList} 
          />
        </View>
        ...
        <View style={styles.section}>
          <View style={styles.cardHeader}>
            ...
            <TouchableOpacity onPress={() => console.log("View detailed stats")}>
              <Ionicons name="stats-chart" size={18} color={COLORS.primary} />
            </TouchableOpacity>
          </View>
          <View style={styles.chartContainer}>
            <View style={styles.chartBars}>
              {STATS_CHART.map((hour, index) => renderChartBar({ item: hour, index }))}
            </View>
            <View style={styles.chartLegend}>
              <View style={styles.legendItem}>
                <View style={[styles.legendDot, { backgroundColor: COLORS.primary }]} />
                <ThemedText style={styles.legendText}>Tests Processed</ThemedText>
              </View>
            </View>
          </View>
        </View>
        ...
        <View style={styles.section}>
          <View style={styles.cardHeader}>
            ...
            <TouchableOpacity onPress={() => console.log("Refresh equipment status")}>
              <Ionicons name="refresh" size={18} color={COLORS.primary} />
            </TouchableOpacity>
          </View>
          <View style={styles.equipmentGrid}>
            {EQUIPMENT_STATUS.map((equipment, index) => renderEquipment({ item: equipment, index }))}
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  kpiCard: { 
    ...
    width: isSmallScreen ? width * 0.75 : 280, 
    marginRight: 16, 
    shadowColor: "#000", 
    shadowOffset: { width: 0, height: 4 }, 
    shadowOpacity: 0.1, 
  ...
  criticalCard: { 
    ...
    borderRadius: 16, 
    padding: 24, 
    width: isSmallScreen ? width * 0.85 : 350, 
    marginRight: 20, 
    borderWidth: 1, 
    borderColor: "#FECACA",
    ...(Platform.OS === 'web' && {
