...
export default function PharmacyProfileMenuModal({
  visible,
  onClose,
}: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (session?.name) {
        setPharmacistName(session.name);
      }

      if (session?.uid) {
        setPharmacistId(buildEntityId(session.uid, "PHM"));
      }
    })();
  }, [visible]);
    ...
    <Modal transparent visible={visible} animationType="none">
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={styles.name}>{pharmacistName}</Text>
            {!!pharmacistId && <Text style={styles.id}>{pharmacistId}</Text>}
        ...
        <MenuRow
          ...
          onPress={() => {
            onClose();
            router.push("/pharmacy/security/password");
          }}
        />
