...
export default function HospitalProfileMenuModal({
  visible,
  onClose,
}: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (session?.name) {
        setHospitalName(session.name);
      }

      if (session?.uid) {
        setHospitalId(buildEntityId(session.uid, "HOS"));
      }
    })();
  }, [visible]);
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <View style={{ flex: 1 }}>
            ...
            {!!hospitalId && <Text style={styles.id}>{hospitalId}</Text>}
        ...
        <MenuRow
          ...
          onPress={() => {
            onClose();
            router.push("/hospital/security/password");
          }}
        />
