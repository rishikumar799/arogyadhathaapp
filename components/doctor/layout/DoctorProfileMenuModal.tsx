...
// Matches DoctorTopNav total height on mobile
...
export default function DoctorProfileMenuModal({ visible, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (session?.uid) {
        setDoctorId(buildEntityId(session.uid, "DOC"));
      }
    })();
  }, [visible]);
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <View style={{ flex: 1 }}>
            ...
            {doctorId ? <Text style={styles.id}>{doctorId}</Text> : null}
        ...
        <MenuRow
          ...
          onPress={() => {
            onClose();
            router.push("/doctor/security/password");
          }}
        />
