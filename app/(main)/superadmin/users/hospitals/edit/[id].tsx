...
export default function EditHospitalPage() {
      ...
      if (newStatus !== oldStatus) {
        ...
        if (newStatus === "approved") {
          await Promise.all([
            ...
            setDoc(doc(db, "hospitals", id), payload),
            deleteDoc(doc(db, "requests", id)),
            setDoc(doc(db, "hospitalConfigs", id), {
        ...
        if (newStatus === "pending") {
          await Promise.all([
            ...
            deleteDoc(doc(db, "hospitals", id)),
            deleteDoc(doc(db, "hospitalConfigs", id)),
          ]);
        }
      } else {
        // ✏️ NO STATUS CHANGE
        ...
        if (userData.__source === "users") {
          await updateDoc(doc(db, "hospitals", id), form);
        }
      }
  ...
  if (loading) {
    return (
      <View style={styles.center}>
        <ThemedText>Loading hospital…</ThemedText>
      </View>
    );
  }
  ...
  return (
    <ScrollView style={styles.wrapper}>
      ...
      <ThemedText style={styles.title}>Edit Hospital</ThemedText>

      <ReadOnly label="Email" value={userData.email} />
      <ReadOnly label="Role" value="hospital" />
