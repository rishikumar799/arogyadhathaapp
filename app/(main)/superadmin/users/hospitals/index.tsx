...
export default function HospitalsPage() {
  ...
  const [activeHospitals, setActiveHospitals] = useState<User[]>([]);
  const [deletedHospitals, setDeletedHospitals] = useState<User[]>([]);
 ...
 useEffect(() => {
  ...
  useEffect(() => {
    ...
    return onSnapshot(q, snap => {
      const deleted: User[] = snap.docs
        .map(d => ({
          id: d.id,
          deleted: true,
          ...(d.data() as Omit<User, "id" | "deleted">),
        }))
        .filter(
          u =>
            String(u.role || "")
      ...
      setDeletedHospitals(deleted);
    });
  }, []);

  /* ================= MERGE + SEARCH ================= */

  const hospitals = useMemo<User[]>(() => {
    const deletedMap = new Map<string, User>();
    deletedHospitals.forEach(d => deletedMap.set(d.id, d));

    const merged: User[] = [];

    activeHospitals.forEach(d => {
      if (!deletedMap.has(d.id)) merged.push(d);
    });

    deletedHospitals.forEach(d => merged.push(d));
  ...
  }, [activeHospitals, deletedHospitals, search]);
              ...
              {!d.deleted && (
                <>
                  <IconBtn
                    ...
                    onPress={() =>
                      router.push(
                        `superadmin/users/hospitals/view/${d.id}`
                      )
                    }
                  />
                  <IconBtn
                    ...
                    onPress={() =>
                      router.push(
                        `superadmin/users/hospitals/edit/${d.id}`
                      )
                    }
                  />
                  <IconBtn
