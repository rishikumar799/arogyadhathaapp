 ...
 const save = async () => {
    ...
    if (newStatus !== oldStatus) {
      ...
      if (newStatus === "approved") {
        await Promise.all([
          ...
          setDoc(doc(db, "receptionists", id), payload),
      ...
      if (newStatus === "pending") {
        await Promise.all([
          ...
          deleteDoc(doc(db, "receptionists", id)),
        ]);
      }
    } else {
      // NO STATUS CHANGE → UPDATE CURRENT SOURCE
      await updateDoc(doc(db, userData.__source, id), form);

      // ALSO UPDATE ROLE COLLECTION IF APPROVED
      if (userData.__source === "users") {
        await updateDoc(doc(db, "receptionists", id), form);
      }
    }
  ...
  if (loading) {
    return (
      <View style={styles.center}>
        <ThemedText>Loading receptionist…</ThemedText>
      </View>
    );
  }
  ...
  return (
    <ScrollView style={styles.wrapper}>
      ...
      <ReadOnly label="Role" value="receptionist" />
